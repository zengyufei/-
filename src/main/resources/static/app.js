...
function renderSchedule(timetable) {
  ...
  $("#score").text("分数: " + (timetable.score == null ? "?" : timetable.score));
  ...
  headerRowByRoom.append($("<th>时间段</th>"));
  ...
  headerRowByTeacher.append($("<th>时间段</th>"));
  ...
  headerRowByStudentGroup.append($("<th>时间段</th>"));
...
function analyze() {
  ...
  if (loadedSchedule.score == null || loadedSchedule.score.indexOf('init') != -1) {
    scoreAnalysisModalContent.text("尚无可供分析的分数，请先按“解决”按钮。");
  } else {
    ...
    $.put("/timetables/analyze", JSON.stringify(loadedSchedule), function (scoreAnalysis) {
      ...
      const analysisTHead = $(`<thead/>`).append($(`<tr/>`)
        .append($(`<th></th>`))
        .append($(`<th>约束</th>`).css({textAlign: 'left'}))
        .append($(`<th>硬/软约束</th>`))
        .append($(`<th># 匹配</th>`))
        .append($(`<th>权重</th>`))
        .append($(`<th>分值</th>`))
